...
export class CompostajeComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder, private compostajeService: CompostajeMockService, private toastService: ToastService) { this.filtroForm = this.fb.group({ busqueda: [''], estadoSeleccionado: [''] }); }
  ...
  eliminarCompostaje(registro: Compostaje): void { if (confirm(`¿Está seguro de eliminar el registro ${registro.lote}? Esta acción no se puede deshacer.`) && this.compostajeService.eliminarCompostaje(registro.id)) { this.actualizarListado(); this.toastService.danger('Registro de compostaje eliminado correctamente'); } }
